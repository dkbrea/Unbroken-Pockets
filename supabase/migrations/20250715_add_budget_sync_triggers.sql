-- Migration: Add triggers to automatically sync budget_entries with budget_transactions
-- Description: This migration adds database triggers to ensure budget_entries.spent column 
--              is automatically updated when transactions with budget_category_id are added,
--              edited, or removed.
...
DROP TRIGGER IF EXISTS after_budget_transaction_insert ON budget_transactions;
CREATE TRIGGER after_budget_transaction_insert
...
DROP TRIGGER IF EXISTS after_budget_transaction_update ON budget_transactions;
CREATE TRIGGER after_budget_transaction_update
...
DROP TRIGGER IF EXISTS after_budget_transaction_delete ON budget_transactions;
CREATE TRIGGER after_budget_transaction_delete
...
-- Run an initial synchronization once after applying the migration
DO $$
DECLARE
  transaction_record RECORD;
BEGIN
  -- Process all existing transactions with budget_category_id
